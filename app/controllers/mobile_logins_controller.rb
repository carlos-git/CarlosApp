class MobileLoginsController < ApplicationController
  ...
  def sign_up
          if request.post?
              ...
              params[:user][:user_ip] = "0.0.0.0"
              params[:user][:email] = params[:user_email] 
              params[:user][:password] = params[:user_password]
              @user = User.new(params[:user])
             # @user.save
                if @user.save
                  render :json => { :success => true, :status => 1, :msg => "Sign up Successful"  }
                else
                 render :json => { :success => true, :status => 0, :msg => "Sign up Failed"  }
                end
