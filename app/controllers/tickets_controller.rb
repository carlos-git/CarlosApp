...
{  :state=>"0", 
   :status=>"1", 
   :message=>"Approved BankAuth: 826914443 CvvResult: M CVVCodeReason: CVV2 Match", 
    :technical=>nil, 
    :transaction_id=>"b999d3b1-2507-4185-99e5-a2fe0cc9873a"
}
=end
  ...
  def allied
     #render :text => params[:allied].to_json and return
    @user_data = params[:allied]
    @user_data[:ipaddress] = request.remote_ip 

      
    @retur = Allied.execute_credit_card2(@user_data)
    
   # render :text => @retur[:message] and return
    
       @status= @retur[:status];
        
     #   @pay_gross=params['mc_gross']
        
     #   @paypal_fees = params['mc_fee'] # Added on 29th Aug
        
        @payee_email= @user_data[:email]
        @txn_id= @retur[:transaction_id]
        @pay_id = @user_data[:id]
        ...
        if( @status =='1'  &&  @chk_transaction_id==0)
           # UserMailer.confirm_paypal(@status,@pay_gross,@custom_amt,@chk_transaction_id,@txn_id,@payee_email,@pay_id).deliver
            Purchase.update_all(['txn_id=?, payer_email=?', @txn_id, @payee_email], ['id=? or transaction_key=?', @pay_id, @pay_id])
            purchase_update(@pay_id, "allied", @paypal_fees)   # Send purchase id and type of payment as parameters. 
                     
            flash[:notice1] = I18n.t 'event.purchase.pur_suc'
               redirect_to(:action=>'order_confirmed/'+@pay_id)
     ...
     user_id = params[:user_id]
      ...
      # If response code is 1, trxn is successful.
       ...
       if Purchase.exists?(params[:pur_id])
           purchase_update(params[:pur_id], "auth_net",0.25) # Send purchase id and type of payment as parameters.
 ...
 # -------------------- PAYPAL --------------------------#
  ...
  def paypal_ipn
        ...
        if((@status.downcase =='completed' || @status.downcase=='pending') && (@pay_gross.to_f==@custom_amt.to_f || @pay_gross.to_f>@custom_amt.to_f) && @chk_transaction_id==0)
            ...
            Purchase.update_all(['txn_id=?, payer_email=?,gateway=?', @txn_id, @payee_email, 'allied'],  ['id=? or transaction_key=?', @pay_id.to_i, @pay_id.to_i])
           
            purchase_update(@txn_id, "paypal", @paypal_fees)   # Send purchase id and type of payment as parameters. 
            puts ">>>>> #{params[:mc_fee]}"
...
#        UserMailer.confirm_order_attendee(order,@pur1).deliver            
 ...
 def order_confirmed
  ...
  def purchase_update(id, type, paypal_fees)
